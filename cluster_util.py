...
def generate_DBSCAN(vector, labels, eps, min_samples, twoD_vector):
    ...
    n_clusters_ = len(set(cluster_DBSCAN_Labels)) - (
        1 if -1 in cluster_DBSCAN_Labels else 0
    )
    ...
    print(
        "Homogeneity: %0.3f" % metrics.homogeneity_score(labels, cluster_DBSCAN_Labels)
    )
    print(
        "Completeness: %0.3f"
        % metrics.completeness_score(labels, cluster_DBSCAN_Labels)
    )
    print("V-measure: %0.3f" % metrics.v_measure_score(labels, cluster_DBSCAN_Labels))
    print(
        "Adjusted Rand Index: %0.3f"
        % metrics.adjusted_rand_score(labels, cluster_DBSCAN_Labels)
    )
    print(
...
def generate_AgglomerativeCluster(
    vector, labels, twoD_vector, n_cluster=2, linkage="ward"
):
    _, ndim = vector.shape
    clustering = AgglomerativeClustering(n_clusters=n_cluster, linkage=linkage).fit(
        vector
    )
...
def generate_spectral_clustering(
    vector, labels, twoD_vector, n_cluster=2, assign_labels="discretize"
):
    _, ndim = vector.shape
    clustering = SpectralClustering(
        n_clusters=n_cluster, assign_labels=assign_labels, random_state=0
    ).fit(vector)
...
def generate_kmeans_clustering(
    vector, labels, twoD_vector, n_cluster=2, random_state=0
):
    _, ndim = vector.shape
...
def model_evaluation(X, labels):
    ...
    d = {
        "Silhoutte": silhoutte,
        "Calinski_Harbasz": calinski_harabasz,
